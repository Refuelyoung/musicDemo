...
function str2binl(str) {
    ...
    for (var i = 0; i < str.length * chrsz; i += chrsz) bin[i >> 5] |= (str.charCodeAt(i / chrsz) & (1 << chrsz) - 1) << i % 32;
    return bin;
}
...
function binl2b64(binarray) {
    ...
    for (var i = 0; i < binarray.length * 4; i += 3) {
        ...
        for (var j = 0; j < 4; j++) {
            if (i * 8 + j * 6 > binarray.length * 32) str += b64pad;else str += 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'.charAt(triplet >> 6 * (3 - j) & 0x3F);
        }
    }
    return str;
}
